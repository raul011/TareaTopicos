...
public sealed class WorkerService
{
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Procesar negocio
                    await ProcessTransactionWithScopeAsync(tx, ct);
    ...
    private async Task ProcessTransactionWithScopeAsync(Transaccion tx, CancellationToken ct)
    {
        // Crea un scope nuevo para esta transacción específica.
        using (var scope = _scopeFactory.CreateScope())
        {
            var processor = scope.ServiceProvider.GetRequiredService<IQueueProcessor>();
            await processor.ProcessAsync(tx, ct);
        }
    }
}
